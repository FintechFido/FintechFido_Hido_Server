                ...
                if (dbimei == hash_imei && dbBankCode == running) {
                    ...
                    connection.query(sql2, [CI, running, saved, hash_session_key, enChallengeNum], function (error, results) {
                            if (error) throw error;
                                ...
                                console.log("FINGERPRINT VAILD - SUCCESS  /  Challenge number [" + enChallengeNum + "]");
                                var output = {
...
app.post("/auth", function (req, res) {
    ...
    var hash_sessionKey = (crypto.createHash('sha512').update(String(session_key)).digest('base64')); //해시

    //certificationDB에서 useBankCode, sessionKey 검색해서 CI획득
    var sql = "SELECT * FROM certification WHERE useBankCode = ? AND sessionKey = ?";
    connection.query(sql, [running, hash_sessionKey], function (error, results) {
        if (error) throw error;
        else {
            var CI = results[0].CI;
            let option = {
